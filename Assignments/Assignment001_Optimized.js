...
//============================================================
const dataSet = require("../TestData/Assignment_Data_Optimized.json");
...
dataSet.forEach(data => {
  test(`Testing with User/Password: ${data.Name}/${data.Password}`, async t => {
    ...
    //Step#4.1: If the login credentials are invalid, Assert the error message (Presence & color) for wrong login
    if (errorMessage.exists == true) {
      await t.expect(errorMessage.getStyleProperty('color')).eql(hexToRgb(errColor))
    }
    else if (errorMessage.exists == false) //Step4.2: else if the login is valid, Assert the existence of any of the homepage contents
    // (in our case, the acccout summary tab)
    {
      await t.expect(accntSummaryTab.exists).ok();
    }
